...
export default {
  ...
  methods: {
    ...
    confirmConnectAlert({ name, did, role, from }) {
      console.log({ did: this.did });
      this.$buefy.dialog.confirm({
        title: "Received Connection Request",
        message: `Received connection request from account with Name: <b>${name}</b>, DID: <b>${did}</b> and has the role of <b>${role}</b>. Are you sure you want to <b>connect</b> and share details of your account? This action will start new session.`,
    ...
    onConfirmConnect({ from, did, name, role }) {
      this.$store.dispatch("acceptConnection", { name, role, from, did });
      this.success();
    },
    afterConnectionIsAccepted({ uName, uDid, encMsg, from }) {
      this.isLoading = false;
      this.success();
      this.$store.dispatch("connectionAccepted", {
        from,
        did: uDid,
        name: uName,
        encMsg,
  ...
  created() {
    ...
    socket.on("connect request", ({ uName, uDid, uRole, from }) => {
      ...
      this.confirmConnectAlert({ name: uName, did: uDid, role: uRole, from });
    });
